...
const ConfirmationPage: React.FC = () => {
  const { subtotal, shippingCharge, tax, discount, totalAmount, billingAddress, shippingAddress, selectedPaymentMethod, cartItems } = useCart();
  ...
  return (
    <div className='container mx-auto px-4 py-8'>
      <header className='bg-gray-800 text-white py-4 text-center'>
        <h1 className='text-3xl font-bold'>Bajar</h1>
        <p className='mt-2 text-lg'>Order Confirmation</p>
      </header>
      ...
      <div className='bg-white p-6 rounded-lg shadow-lg mt-8'>
        ...
        <div className='mb-6'>
          ...
          <p className='text-gray-800'>{selectedPaymentMethod.replace(/-/g, ' ').toUpperCase()}</p>
        </div>
      ...
      <div className='text-center mt-8'>
        ...
        <Button className='ml-4'>
          <a href='#' onClick={() => window.print()}>Print Receipt</a>
        </Button>
      </div>
    </div>
  );
